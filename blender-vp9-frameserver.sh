...
} | ffmpeg -vcodec ppm -f image2pipe -r $R_rate -i pipe:0 -vcodec libvpx-vp9 -crf 30 -b 0 -threads $THREADS $OUTPUT
# for audio addition uncomment the line below and set the AUDIO variable at the top
#} | ffmpeg -vcodec ppm -f image2pipe -r $R_rate -i pipe:0 -i $AUDIO -vcodec libvpx-vp9 -crf 30 -b 0 -acodec libopus -vbr on -threads $THREADS $OUTPUT
